...
## Ad removal (Megaphone shows only)

Set `strip_ads: true` on a podcast in `podcasts.yaml` for shows served
through Megaphone's dynamic ad insertion (confirmed for Stinky Dragon:
re-downloading the same episode twice gave two different durations,
since Megaphone stitches a different ad into each request). For those
shows, each episode is downloaded *twice* and the two copies are diffed
(`ad_strip.py`) to find and cut whatever audio is unique to one copy --
real show content is identical between downloads, so anything that
isn't is presumed to be an ad. See `ad_strip.py`'s module docstring for
how the diffing works and its real limitations (only catches mid-roll-
style ads confidently bounded on both sides; won't catch an ad that
happens to be identical between both downloads; roughly doubles
download time per episode it's set on). If the two downloads don't
align confidently enough to trust a cut, it falls back to the full,
unedited audio rather than guessing.
